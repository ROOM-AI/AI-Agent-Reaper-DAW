...
function export_state()
    ...
    for i = 0, numTracks - 1 do
        ...
        -- Export Volume automation if it exists
        ...
        if volEnv then
            ...
            if numPoints > 0 then
                ...
                local maxShow = math.min(numPoints, 10)
                for p = 0, maxShow - 1 do
                    local _, time, value, _, _ = reaper.GetEnvelopePoint(volEnv, p)
                    local volDB = 20 * math.log(value, 10)
                    stateFile:write(string.format("    Point %d: %.2fs = %.1fdB\n", p, time, volDB))
                end
                if numPoints > maxShow then
                    stateFile:write(string.format("    ... and %d more points\n", numPoints - maxShow))
                end
            end
        end
    end
