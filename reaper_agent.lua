...
-- Load once: Actions ‚Üí Show action list ‚Üí New action ‚Üí Load ReaScript ‚Üí select this file ‚Üí Run
...
function execute_with_feedback(command_name, success, message)
    if success then
        local feedback_msg = string.format("‚úì %s: %s", command_name, message or "success")
        msg(feedback_msg)
        add_feedback(feedback_msg)
        return true
    else
        local feedback_msg = string.format("‚úó %s: %s", command_name, message or "failed")
        msg(feedback_msg)
        add_feedback(feedback_msg)
        return false
    end
...
function process_command(line)
    ...
    if cmd == "LIST_PLUGINS" then
        ...
        msg(string.format("Ì†ΩÌ≥ã Listed %d Reaper stock plugins to state file", #plugins))
        add_feedback(string.format("‚úì Listed %d Reaper stock plugins", #plugins))
        return
    end
    ...
    if cmd == "SEARCH_PLUGIN" then
        ...
        msg(string.format("Ì†ΩÌ¥ç Found %d Reaper stock plugins matching '%s'", #matches, searchTerm))
        add_feedback(string.format("‚úì Found %d Reaper stock plugins matching '%s'", #matches, searchTerm))
        return
    end
    ...
    if actionID then
        ...
        msg("‚úì Executed action: " .. actionID)
        add_feedback("‚úì Action " .. actionID .. " executed")
        return
    end
    ...
    if cmd == "VOL_DIP" then
        ...
        if env then
            ...
            execute_with_feedback("VOL_DIP", true, string.format("Track %d: %.1fs‚Üí%.1fs at %.0f%%", trackIdx, tStart, tEnd, value*100))
        else
    ...
    elseif cmd == "INSERT_INSTRUMENT" then
        ...
        if fxIdx >= 0 then
            ...
            local successMsg = string.format("Ì†ºÌæπ Inserted instrument '%s' on track %d (matched '%s')", finalName ~= "" and finalName or instName, trackIdx, matchedName or instName)
            if midiSet then
                successMsg = successMsg .. string.format(" | MIDI input: %s", midiSource or "All inputs")
            else
                successMsg = successMsg .. " | ‚ö†Ô∏è MIDI input not set (no MIDI devices found)"
            end
            msg(successMsg)
            add_feedback(successMsg)
        else
            -- Instrument not found - try fallback instruments based on type
            msg(string.format("‚ö†Ô∏è '%s' not found, searching for similar instrument...", instName))
            ...
            if fallbackIdx >= 0 then
                ...
                local successMsg = string.format("Ì†ºÌæπ Inserted FALLBACK '%s' on track %d (requested '%s' not found)", finalName ~= "" and finalName or fallbackName, trackIdx, instName)
                if midiSet then
                    successMsg = successMsg .. string.format(" | MIDI input: %s", midiSource or "All inputs")
                end
                msg(successMsg)
                ...
                local failMsg = string.format("‚ùå Could not find instrument '%s' or any fallback", instName)
                msg(failMsg)
    ...
    elseif cmd == "ADD_FX" then
        ...
        if track then
            ...
            if existingIdx >= 0 then
                ...
                local successMsg = string.format("Ì†ºÌæõÔ∏è Opened existing FX '%s' on track %d", existingName, trackIdx)
                msg(successMsg)
                add_feedback(successMsg)
            else
                ...
                if fxIdx >= 0 then
                    ...
                    local successMsg = string.format("Ì†ºÌæõÔ∏è Added FX '%s' (matched as '%s') to track %d", fxName, matchedName, trackIdx)
                    msg(successMsg)
                    add_feedback(successMsg)
                else
                    local failMsg = string.format("‚ùå Could not find FX: %s", fxName)
                    msg(failMsg)
                    add_feedback(failMsg)
                end
            end
    ...
    elseif cmd == "SET_FX_PARAM" then
        ...
        if track then
            ...
            if fxIdx < numFX then
                ...
                local successMsg = string.format("Ì†ºÌæöÔ∏è Track %d FX#%d '%s' - %s ‚Üí %.0f%% [%s]", trackIdx, fxIdx, fxName, paramName, value*100, displayValue)
                msg(successMsg)
                add_feedback(successMsg)
            else
                local failMsg = string.format("‚ùå Track %d only has %d FX (tried to access FX#%d)", trackIdx, numFX, fxIdx)
                msg(failMsg)
                add_feedback(failMsg)
            end
        end
    ...
    elseif cmd == "EQ_NEUTRALIZE_BAND" then
        ...
        local track = reaper.GetTrack(0, trackIdx)
        if not track then
            msg(string.format("‚ùå Track %d not found", trackIdx))
            add_feedback(string.format("‚ùå Track %d not found", trackIdx))
            return
        end
        
        local numFX = reaper.TrackFX_GetCount(track)
        if fxIdx >= numFX then
            msg(string.format("‚ùå Track %d only has %d FX (tried FX#%d)", trackIdx, numFX, fxIdx))
            add_feedback(string.format("‚ùå Track %d only has %d FX (tried FX#%d)", trackIdx, numFX, fxIdx))
            return
        end
        ...
        if not fxLower:find("pro-q") then
            msg(string.format("‚ö†Ô∏è EQ_NEUTRALIZE_BAND currently only supports Pro-Q (fx: %s)", fxName))
            add_feedback(string.format("‚ö†Ô∏è Skipped neutralizing band on %s (unsupported EQ)", fxName))
            return
        end
        ...
        msg(string.format("Ì†æÌ∑º Neutralized Pro-Q band %d on track %d FX#%d", bandNum, trackIdx, fxIdx))
        add_feedback(string.format("‚úì Neutralized Pro-Q band %d (track %d)", bandNum, trackIdx))
    ...
    elseif cmd == "REMOVE_FX" then
        ...
        if track then
            ...
            if fxIdx < numFX then
                ...
                local successMsg = string.format("Ì†ΩÌ∑ëÔ∏è Removed FX#%d '%s' from track %d", fxIdx, fxName, trackIdx)
                msg(successMsg)
                add_feedback(successMsg)
            else
                local failMsg = string.format("‚ùå Track %d only has %d FX (tried to remove FX#%d)", trackIdx, numFX, fxIdx)
                msg(failMsg)
                add_feedback(failMsg)
            end
        end
    ...
    elseif cmd == "SET_TRACK_VOL" then
        ...
        if track then
            ...
            execute_with_feedback("SET_TRACK_VOL", true, string.format("Track %d ‚Üí %.1fdB", trackIdx, volDB))
        else
        ...
        if track then
            ...
            if env then
                ...
                if numPoints > 0 then
                    ...
                    local successMsg = string.format("Ì†ΩÌ∑ëÔ∏è Cleared %d automation points from %s on track %d", numPoints, envName, trackIdx)
                    msg(successMsg)
                    add_feedback(successMsg)
                else
                    local infoMsg = string.format("‚ÑπÔ∏è No automation points on %s envelope of track %d", envName, trackIdx)
                    msg(infoMsg)
                    add_feedback(infoMsg)
                end
            else
                local failMsg = string.format("‚ùå No %s envelope on track %d", envName, trackIdx)
                msg(failMsg)
                add_feedback(failMsg)
            end
        else
            local failMsg = string.format("‚ùå Track %d not found", trackIdx)
            msg(failMsg)
            add_feedback(failMsg)
        end
    ...
    elseif cmd == "CLEAR_AUTOMATION_RANGE" then
        ...
        if track then
            ...
            if env then
                ...
                local successMsg = string.format("Ì†æÌ∑π Cleared automation range %.2fs‚Äì%.2fs on %s (track %d)", tStart, tEnd, envName, trackIdx)
                msg(successMsg)
                add_feedback(successMsg)
            else
                local failMsg = string.format("‚ùå No %s envelope on track %d", envName, trackIdx)
    ...
    elseif cmd == "MIDI_CLEAR_SELECTION" then
        ...
        local msgText = tStart and string.format("Ì†æÌ∑π Cleared MIDI selection on track %d (%.2f‚Äì%.2fs)", trackIdx, tStart, tEnd)
            or string.format("Ì†æÌ∑π Cleared MIDI selection on track %d", trackIdx)
        msg(msgText)
    ...
    elseif cmd == "MIDI_SELECT_PITCHES" then
        ...
        if #pitchList == 0 then
            msg("‚ùå MIDI_SELECT_PITCHES: no pitches provided")
            return
        end
        ...
        local msgText = tStart and string.format("‚úÖ Selected %d MIDI notes on track %d pitches %s (%.2f‚Äì%.2fs)", selectedCount, trackIdx, parts[3], tStart, tEnd)
            or string.format("‚úÖ Selected %d MIDI notes on track %d pitches %s", selectedCount, trackIdx, parts[3])
    ...
    elseif cmd == "MIDI_SET_VELOCITY_CONST" then
        ...
        local track = reaper.GetTrack(0, trackIdx)
        if not track then
            msg(string.format("‚ùå Track %d not found", trackIdx)); return
        end
        local itemCount = reaper.CountTrackMediaItems(track)
        local changed = 0
        for i = 0, itemCount - 1 do
        ...
        msg(string.format("Ì†ºÌæπ Set velocity=%d on %d MIDI notes (track %d)", velocity, changed, trackIdx))
        add_feedback(string.format("‚úì Set velocity %d on %d notes (track %d)", velocity, changed, trackIdx))
    ...
    elseif cmd == "MIDI_SET_VELOCITY_RAMP" then
        ...
        msg(string.format("Ì†ºÌæõÔ∏è Velocity ramp %d‚Üí%d applied to selected MIDI notes on track %d", startVel, endVel, trackIdx))
        add_feedback(string.format("‚úì Velocity ramp %d‚Üí%d applied to selected notes (track %d)", startVel, endVel, trackIdx))
    ...
    elseif cmd == "MIDI_CREATE_ITEM" then
        ...
        if item then
            msg(string.format("Ì†ºÌæπ Created MIDI item on track %d (%.2fs-%.2fs)", trackIdx, tStart, tEnd))
            add_feedback(string.format("‚úì Created MIDI item on track %d", trackIdx))
        else
            msg("‚ùå Failed to create MIDI item")
            add_feedback("‚ùå Failed to create MIDI item")
        end
    ...
    elseif cmd == "MIDI_ACTION" then
        ...
        if not actionID then
            msg("‚ùå MIDI_ACTION requires a numeric command ID")
            return
        end
        ...
        if editor then
            ...
            local successMsg = string.format("Ì†ºÌæπ Executed MIDI Editor Action %d", actionID)
            msg(successMsg)
            add_feedback("‚úì " .. successMsg)
        else
            msg("‚ùå No active MIDI Editor found. Use MIDI_EDITOR_OPEN first.")
            add_feedback("‚úó No active MIDI Editor")
        end
    ...
    elseif cmd == "MIDI_EDITOR_OPEN" then
        ...
        if item then
            ...
            msg(string.format("Ì†ºÌæπ Opened MIDI editor for item on track %d at %.1fs", trackIdx, time))
            add_feedback(string.format("‚úì Opened MIDI editor (track %d)", trackIdx))
        else
            msg(string.format("‚ùå No MIDI item found on track %d at %.1fs", trackIdx, time))
            add_feedback("‚úó No MIDI item found to open")
        end
    ...
    elseif cmd == "MIDI_INSERT_NOTE" then
        ...
        if not pitch then
            msg(string.format("‚ùå Invalid MIDI note '%s'", tostring(pitchToken)))
            add_feedback(string.format("‚úó Invalid MIDI note '%s'", tostring(pitchToken)))
            return
        end
        ...
        if not take then
            msg("‚ùå Could not access MIDI take")
            add_feedback("‚ùå Could not access MIDI take")
            return
        end
        ...
        add_feedback(string.format("‚úì Inserted note pitch=%d on track %d", pitch, trackIdx))
    ...
    elseif cmd == "MIDI_INSERT_CHORD" then
        ...
        if not track then
            msg(string.format("‚ùå Track %d not found", trackIdx))
            add_feedback(string.format("‚úó Track %d not found", trackIdx))
            return
        end

        local rootPitch = parse_midi_pitch(rootToken)
        if not rootPitch then
            msg(string.format("‚ùå Invalid chord root '%s'", tostring(rootToken)))
            add_feedback(string.format("‚úó Invalid chord root '%s'", tostring(rootToken)))
            return
        end
        ...
        local take = ensure_midi_take(track, tStart, tEnd)
        if not take then
            msg("‚ùå Could not access MIDI take for chord insertion")
            add_feedback("‚ùå Could not access MIDI take for chord insertion")
            return
        end
        ...
        local feedbackMsg = string.format("Ì†ºÌæº Inserted %s chord (%s) on track %d", normalizedQuality or qualityRaw, detail, trackIdx)
        msg(feedbackMsg)
    ...
    elseif cmd == "MIDI_GET_NOTES" then
        ...
        if track then
            ...
            add_feedback(out)
            msg("Ì†ºÌæº Read " .. #note_list .. " notes from track " .. trackIdx)
        end
    ...
    elseif cmd == "MIDI_QUANTIZE" then
        ...
        msg(string.format("Ì†ΩÌ≥è Quantized selected MIDI notes on track %d to %.2f grid (%.0f%%)", trackIdx, grid, strength*100))
        add_feedback(string.format("‚úì Quantized selected notes on track %d", trackIdx))
    ...
    elseif cmd == "MIDI_CORRECT_PITCH" then
        ...
        msg(string.format("Ì†ºÌæ§ Corrected %d notes to %s %s on track %d", fixedCount, keyName, scaleName, trackIdx))
        add_feedback(string.format("‚úì Pitch corrected %d notes to %s %s", fixedCount, keyName, scaleName))
    ...
    elseif cmd == "CLEAR_FX_PARAM_AUTOMATION" then
        ...
        if env then
            ...
            local _, fxName = reaper.TrackFX_GetFXName(track, fxIdx, "")
            local _, paramName = reaper.TrackFX_GetParamName(track, fxIdx, paramIdx, "")
            local successMsg = string.format("Ì†æÌ∑π Cleared automation on %s > %s (p%d) track %d in %.2f‚Äì%.2fs",
                fxName, paramName, paramIdx, trackIdx, tStart, tEnd)
            msg(successMsg); add_feedback(successMsg)
        else
            local infoMsg = string.format("‚ÑπÔ∏è No automation envelope for FX#%d p%d on track %d", fxIdx, paramIdx, trackIdx)
    ...
    elseif cmd == "FX_PARAM_AUTO" then
        ...
        if not env then
            local failMsg = string.format("‚ùå Could not get envelope for param %d on FX#%d", paramIdx, fxIdx)
            msg(failMsg)
            add_feedback(failMsg)
            return
        end
        ...
        local successMsg = string.format("Ì†ºÌæõÔ∏è Automated %s > %s: %.2fs‚Üí%.2fs (%.0f%%‚Üí%.0f%%)", 
    ...
    elseif cmd == "SET_TEMPO" then
        ...
        msg(string.format("Ì†ºÌæµ Set tempo to %.0f BPM", bpm))
        add_feedback(string.format("‚úì Tempo set to %.0f BPM", bpm))
    ...
    elseif cmd == "SET_TIME_SIGNATURE" then
        ...
        msg(string.format("Ì†ºÌæº Set time signature to %d/%d at bar %d", num, denom, bar))
        add_feedback(string.format("‚úì Time signature: %d/%d", num, denom))
    ...
    elseif cmd == "SET_PROJECT_SAMPLERATE" then
        ...
        msg(string.format("Ì†ºÌæõÔ∏è Set sample rate to %d Hz", sr))
        add_feedback(string.format("‚úì Sample rate: %d Hz", sr))
    ...
    elseif cmd == "SET_TRACK_NAME" then
        ...
        if track then
            ...
            msg(string.format("Ì†ΩÌ≥ù Named track %d: %s", trackIdx, name))
            add_feedback(string.format("‚úì Track %d named: %s", trackIdx, name))
        else
            msg(string.format("‚ùå Track %d not found", trackIdx))
            add_feedback(string.format("‚úó Track %d not found", trackIdx))
        end
    ...
    elseif cmd == "SET_TRACK_COLOR" then
        ...
        if track then
            ...
            msg(string.format("Ì†ºÌæ® Set track %d color: RGB(%d,%d,%d)", trackIdx, r, g, b))
            add_feedback(string.format("‚úì Track %d color set", trackIdx))
        else
            msg(string.format("‚ùå Track %d not found", trackIdx))
            add_feedback(string.format("‚úó Track %d not found", trackIdx))
        end
    ...
    elseif cmd == "CREATE_SEND_BUS" then
        ...
        if bus_track then
            ...
            msg(string.format("Ì†ºÌæöÔ∏è Created send bus: %s (track %d)", name, track_count))
            add_feedback(string.format("‚úì Bus created: %s", name))
        end
    ...
    elseif cmd == "SET_TRACK_SEND" then
        ...
        if from_track and to_track then
            ...
            if send_idx >= 0 then
                ...
                msg(string.format("Ì†ΩÌ≥§ Created send: Track %d ‚Üí Track %d (%.1f dB)", fromIdx, toIdx, level_db))
                add_feedback(string.format("‚úì Send: %d‚Üí%d @ %.1fdB", fromIdx, toIdx, level_db))
            else
                msg(string.format("‚ùå Failed to create send"))
                add_feedback(string.format("‚úó Send creation failed"))
            end
        else
            msg(string.format("‚ùå Track not found (from:%d to:%d)", fromIdx, toIdx))
            add_feedback(string.format("‚úó Track not found"))
        end
        ...
        if from_track and to_track then
            ...
            if comp_idx >= 0 then
                ...
                if send_idx >= 0 then
                    ...
                    msg(string.format("Ì†ΩÌ¥ó Sidechain: Track %d ‚Üí Track %d (%.0f%%)", fromIdx, toIdx, amount*100))
                    add_feedback(string.format("‚úì Sidechain: %d‚Üí%d", fromIdx, toIdx))
                end
            end
        else
            msg(string.format("‚ùå Track not found"))
            add_feedback(string.format("‚úó Sidechain failed"))
        end
    ...
    elseif cmd == "SET_SWING" then
        ...
        msg(string.format("Ì†ºÌæµ Swing set to %.1f%%", amount * 100))
        add_feedback(string.format("‚úì Swing: %.1f%%", amount * 100))
    ...
    elseif cmd == "EXPORT_AUDIO" then
        ...
        if track then
            ...
            msg(string.format("Ì†ºÌæµ Exported track %d to %s", trackIdx, outputPath))
            add_feedback(string.format("‚úì Exported track %d to %s", trackIdx, outputPath))
        else
            msg(string.format("‚ùå Track %d not found", trackIdx))
            add_feedback(string.format("‚úó Track %d not found", trackIdx))
        end
    ...
    elseif cmd == "INSERT_AUDIO" then
        ...
        if not f then
            ...
            if f then
                ...
                msg(string.format("‚ùå Audio file not found: %s", filePath))
                add_feedback(string.format("‚úó File not found: %s", filePath))
                return
            end
        end
        f:close()
        ...
        if track then
            ...
            if itemCountAfter > itemCountBefore then
                ...
                if newItem then
                    ...
                    msg(string.format("Ì†æÌµÅ Inserted %s on track %d at %.2fs (%.2fs)", filename, trackIdx, startTime, itemLength))
                    add_feedback(string.format("‚úì Inserted audio: %s at %.2fs", filename, startTime))
                end
            else
                msg(string.format("‚ùå Failed to insert audio: %s", filePath))
                add_feedback(string.format("‚úó Insert failed: %s", filePath))
            end
        else
            msg(string.format("‚ùå Could not create track %d", trackIdx))
            add_feedback(string.format("‚úó Could not create track %d", trackIdx))
        end
    ...
    elseif cmd == "LOAD_SAMPLER" then
        ...
        if not f then
            msg(string.format("‚ùå Sample file not found: %s", filePath))
            add_feedback(string.format("‚úó File not found: %s", filePath))
        else
            f:close()
            ...
            if track then
                ...
                if fxIdx >= 0 then
                    ...
                    msg(string.format("Ì†ºÌæπ Loaded %s into sampler on track %d (notes %d-%d, base=%d)", 
                        filename, trackIdx, noteStart, noteEnd, baseNote))
                    add_feedback(string.format("‚úì Sampler loaded: %s", filename))
                else
                    msg("‚ùå Failed to add ReaSamplOmatic5000")
                    add_feedback("‚úó Failed to add sampler")
                end
            end
        end
    ...
    elseif cmd == "DRUM_SAMPLE" then
        ...
        msg(string.format("‚ö†Ô∏è DRUM_SAMPLE not resolved: track %s, %s at %ss", trackIdx, category, startTime))
        add_feedback(string.format("‚ö†Ô∏è DRUM_SAMPLE needs bridge: %s", category))

    elseif cmd == "ELEVEN_VOCALS" then
        -- Bridge-only command (Python must fetch MP3 + rewrite to INSERT_AUDIO).
        msg("‚ö†Ô∏è ELEVEN_VOCALS reached Lua. Start/restart your Python bridge so it rewrites ELEVEN_VOCALS -> INSERT_AUDIO.")
        add_feedback("‚ö†Ô∏è ELEVEN_VOCALS needs bridge rewrite (cloud_bridge.py/local_bridge.pyw).")

    elseif cmd == "EL1_SONG" then
        -- Bridge-only command (Python must fetch full song + stems + rewrite to INSERT_AUDIO commands).
        msg("‚ö†Ô∏è EL1_SONG reached Lua. Start/restart your Python bridge so it rewrites EL1_SONG -> INSERT_AUDIO commands.")
        add_feedback("‚ö†Ô∏è EL1_SONG needs bridge rewrite (cloud_bridge.py/local_bridge.pyw).")
        
    else
        msg("‚ùì Unknown command: "..tostring(cmd))
    end
end
...
msg("Ì†æÌ¥ñ Reaper AI Agent Started")
